...
% rename for short: 
op = firstPoints_augmented1_init; % old points
np = new_facial_points;
...
ROI_list_points{7} = [np{1}; np{8}; op(74,:); np{9}];
...
ROI_list_points{42} = [op(40,:); op(39,:); op(22,:); op(21,:)];
...
%     regioni_temp = pnts2ROI(ROI_list_points_i, facial_points);
    x1 = [ROI_list_points_i(:, 1)];
    y1 = [ROI_list_points_i(:, 2)];

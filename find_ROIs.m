function [faceROI, included_x_All_Final, included_y_All_Final] = find_ROIs(firstPoints1)
...
firstPoints_augmented_x = [firstPoints2(:,1); forehead1(:,1)]; % add points to the forehead
firstPoints_augmented_y = [firstPoints2(:,2); forehead1(:,2)];
...
TRI = delaunay(firstPoints_augmented(:,1),firstPoints_augmented(:,2));
% figure, imshow(firstFrame), hold on, plot(firstPoints_augmented(:,1),firstPoints_augmented(:,2), '*')
% triplot(TRI,firstPoints_augmented(:,1),firstPoints_augmented(:,2), 'g')
...
while tr_1 < length(included_x_All_Final1)
    if length(included_x_All_Final1{tr_1}) < 100
        ...
        included_y_All_Final{count} = [included_x_All_Final1{tr_1}; included_x_All_Final1{tr_1+1}];
        tr_1 = tr_1+2;
    else 
        count = count+1;
        included_x_All_Final{count} = included_x_All_Final1{tr_1};
        included_y_All_Final{count} = included_x_All_Final1{tr_1};
        tr_1 = tr_1+1;
    end
end